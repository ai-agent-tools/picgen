...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}body{margin:0;background:var(--bg);color:var(--text);font:14px/1.45 -apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}button,input,select,textarea{font:inherit}button{border:1px solid var(--border);background:#fff;color:var(--text);border-radius:7px;padding:7px 10px;cursor:pointer}button.primary{background:var(--accent);border-color:var(--accent);color:#fff}button.ghost{background:transparent}button.danger{color:var(--danger)}button:disabled{opacity:.55;cursor:not-allowed}input,select,textarea{width:100%;border:1px solid var(--border);border-radius:7px;padding:8px 9px;background:#fff;color:var(--text)}textarea{min-height:120px;resize:vertical}.wrap{max-width:1120px;margin:0 auto;padding:22px}.top{display:flex;align-items:flex-start;justify-content:space-between;gap:16px;margin-bottom:18px}.brand h1{font-size:22px;margin:0 0 3px}.brand p,.muted{color:var(--muted);margin:0}.tabs{display:flex;gap:6px;margin-bottom:16px;border-bottom:1px solid var(--border)}.tab{border:0;border-radius:7px 7px 0 0;background:transparent;padding:9px 12px}.tab.active{background:#fff;border:1px solid var(--border);border-bottom-color:#fff;margin-bottom:-1px}.grid{display:grid;gap:14px}.cols{display:grid;grid-template-columns:1fr 360px;gap:14px}.panel{background:var(--panel);border:1px solid var(--border);border-radius:var(--radius);padding:14px}.panel h2{font-size:15px;margin:0 0 12px}.row{display:grid;grid-template-columns:150px 1fr;gap:10px;align-items:center;margin:9px 0}.actions{display:flex;gap:8px;flex-wrap:wrap}.provider{display:grid;grid-template-columns:1fr auto;gap:12px;border:1px solid var(--border);border-radius:var(--radius);padding:12px;background:#fff}.provider+.provider{margin-top:10px}.title{font-weight:650}.badges{display:flex;gap:6px;flex-wrap:wrap;margin-top:6px}.badge{display:inline-flex;align-items:center;border-radius:999px;background:var(--soft);color:#344054;font-size:12px;padding:2px 8px}.badge.ok{background:#dcfae6;color:var(--ok)}.badge.warn{background:#fef0c7;color:var(--warn)}.badge.default{background:#dbeafe;color:#1d4ed8}.keyline{margin-top:8px;color:var(--muted)}.keyline code,.path{font-family:ui-monospace,SFMono-Regular,Menlo,monospace;background:#f2f4f7;border-radius:5px;padding:2px 5px}.formgrid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px}.full{grid-column:1/-1}.status{white-space:pre-wrap;background:#0f172a;color:#e5e7eb;border-radius:var(--radius);padding:10px;max-height:260px;overflow:auto}.plan{font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:12px;white-space:pre-wrap;background:#f8fafc;border:1px solid var(--border);border-radius:var(--radius);padding:10px}.gallery{display:grid;grid-template-columns:repeat(auto-fill,minmax(180px,1fr));gap:12px}.run img,.result img{width:100%;height:150px;object-fit:contain;background:#f8fafc;border:1px solid var(--border);border-radius:var(--radius)}.run{background:#fff;border:1px solid var(--border);border-radius:var(--radius);padding:10px}.hidden{display:none}.toast{position:fixed;right:18px;bottom:18px;background:#111827;color:#fff;border-radius:8px;padding:10px 12px;max-width:360px}.small{font-size:12px}.split{display:flex;justify-content:space-between;gap:8px;align-items:center}@media(max-width:860px){.cols{grid-template-columns:1fr}.row{grid-template-columns:1fr}.formgrid{grid-template-columns:1fr}.top{display:block}.wrap{padding:14px}}
  </style>
</head>
<body>
  <main class="wrap">
    <div class="top">
      <div class="brand"><h1>PicGen</h1><p>Local image generation workspace</p></div>
  ...
  <script>
    ...
    document.querySelectorAll('.tab').forEach(b=>b.onclick=()=>tab(b.dataset.tab));$('#refresh').onclick=()=>load().then(()=>toast('Refreshed')).catch(e=>toast(e.message));
    function renderSettings(){const s=state.data;$('#settings').innerHTML=\`
      <div class="panel"><h2>Paths</h2><div class="row"><div>Config</div><div class="path">\${esc(s.config_path)}</div></div><div class="row"><div>Keys</div><div class="path">~/.picgen/.env</div></div></div>
      <div class="panel"><div class="split"><h2>Providers</h2><button class="primary" id="addProviderBtn">Add Provider</button></div><div id="providers"></div></div>
      <div class="panel hidden" id="providerForm"><h2>Add Provider</h2>
        <div class="formgrid">
          <label>Protocol<select id="newProtocol"><option value="gemini">Gemini</option><option value="openai-images">OpenAI-compatible</option></select></label>
          <label>Channel<select id="newChannel"><option value="third_party">Third-party</option><option value="official">Official</option></select></label>
          <label>Name<input id="newName" placeholder="auto"></label>
          <label>Host<input id="newHost" value="https://www.pandai.vip"></label>
          <label class="full">API key<input id="newKey" type="password" placeholder="Stored locally, never logged"></label>
          <label class="full">Models<input id="newModels" placeholder="recommended defaults"></label>
          <label><input id="newPrefer" type="checkbox" checked style="width:auto"> Set as default</label>
          <div class="actions"><button class="primary" id="saveProvider">Save Provider</button><button id="cancelProvider">Cancel</button></div>
        </div>
      </div>\`;
      $('#addProviderBtn').onclick=()=>$('#providerForm').classList.remove('hidden');$('#cancelProvider').onclick=()=>$('#providerForm').classList.add('hidden');$('#saveProvider').onclick=saveProvider;
      $('#providers').innerHTML=s.providers.map(providerCard).join('') || '<p class="muted">No providers configured.</p>';
      bindProviderActions();
    }
    function providerCard(p){const key=p.key||{set:false};return \`<div class="provider"><div><div class="title">\${esc(p.name)}</div><div class="badges"><span class="badge \${p.preference==='default'?'default':''}">\${esc(p.preference)}</span><span class="badge">\${esc(p.protocol)}</span><span class="badge">\${esc(p.channel)}</span><span class="badge \${p.enabled?'ok':'warn'}">\${p.enabled?'enabled':'disabled'}</span></div><div class="keyline">\${esc(p.base_url)}</div><div class="keyline">Key <code>\${esc(p.api_key_env)}</code>: \${key.set?\`<code>\${esc(key.preview)}</code> <button data-reveal="\${esc(p.api_key_env)}">eye</button>\`:'missing'} \${key.fingerprint?'<span class="small">fingerprint '+esc(key.fingerprint)+'</span>':''}</div></div><div class="actions"><button data-test="\${esc(p.name)}">Test</button><button data-default="\${esc(p.name)}">Default</button><button data-toggle="\${esc(p.name)}">\${p.enabled?'Disable':'Enable'}</button><button class="danger" data-delete="\${esc(p.name)}">Remove</button></div></div>\`}
    function bindProviderActions(){document.querySelectorAll('[data-test]').forEach(b=>b.onclick=()=>testProvider(b.dataset.test));document.querySelectorAll('[data-default]').forEach(b=>b.onclick=()=>post('/api/providers/'+encodeURIComponent(b.dataset.default)+'/default',{}));document.querySelectorAll('[data-toggle]').forEach(b=>{const p=state.data.providers.find(x=>x.name===b.dataset.toggle);b.onclick=()=>patch('/api/providers/'+encodeURIComponent(p.name),{enabled:!p.enabled})});document.querySelectorAll('[data-delete]').forEach(b=>b.onclick=()=>confirm('Remove provider?')&&del('/api/providers/'+encodeURIComponent(b.dataset.delete)));document.querySelectorAll('[data-reveal]').forEach(b=>b.onclick=()=>revealKey(b.dataset.reveal,b))}
    async function saveProvider(){await post('/api/providers',{protocol:$('#newProtocol').value,channel:$('#newChannel').value,name:$('#newName').value,base_url:$('#newHost').value,api_key:$('#newKey').value,models:$('#newModels').value,prefer:$('#newPrefer').checked});$('#providerForm').classList.add('hidden');toast('Provider saved')}
    async function testProvider(name){const r=await api('/api/providers/'+encodeURIComponent(name)+'/test',{method:'POST'});toast((r.ok?'OK ':'Failed ')+r.message)}
    async function revealKey(name,btn){const r=await api('/api/key/'+encodeURIComponent(name));btn.outerHTML='<code>'+esc(r.value||'')+'</code>'}
    ...
    function renderGenerate(){const s=state.data;const prefs=JSON.parse(localStorage.getItem('picgen:prefs')||'{}');$('#generate').innerHTML=\`<div class="cols"><div class="panel"><h2>Generate</h2><label>Prompt<textarea id="prompt">\${esc(prefs.prompt||'一张简洁的 PicGen 测试图，白色背景，少量蓝绿色科技感点缀')}</textarea></label><div class="formgrid"><label>Provider<select id="genProvider"><option value="">Auto</option>\${s.providers.map(p=>'<option value="'+esc(p.name)+'" '+(prefs.provider===p.name?'selected':'')+'>'+esc(p.name)+'</option>').join('')}</select></label><label>Preset<select id="genPreset">\${Object.keys(s.presets).map(p=>'<option '+((prefs.preset||'fast-draft')===p?'selected':'')+'>'+esc(p)+'</option>').join('')}</select></label><label>Model<input id="genModel" value="\${esc(prefs.model||'')}" placeholder="optional; Gemini smoke test uses flash"></label><label>Mode<input id="genMode" value="\${esc(prefs.mode||'')}" placeholder="optional"></label></div><div class="actions" style="margin-top:12px"><button id="preview" class="primary">Preview</button><button id="generateBtn" disabled>Generate</button></div></div><div class="panel"><h2>Plan / Result</h2><div id="plan" class="plan">No preview yet.</div><div id="result" class="result" style="margin-top:12px"></div></div></div>\`;$('#preview').onclick=preview;$('#generateBtn').onclick=generateNow}
    ...
    async function generateNow(){if(!state.plan&&!confirm('Generate without preview?'))return;$('#generateBtn').disabled=true;$('#result').innerHTML='<p class="muted">Generating...</p>';try{const r=await api('/api/generate',{method:'POST',body:JSON.stringify(genBody())});$('#result').innerHTML='<p>Saved to <span class="path">'+esc(r.output_dir)+'</span></p>'+r.images.map(img=>'<img src="/api/file?path='+encodeURIComponent(img.path)+'&token='+token+'"><div class="path">'+esc(img.path)+'</div>').join('');await renderHistory()}catch(e){$('#result').innerHTML='<p style="color:var(--danger)">'+esc(e.message)+'</p>'}finally{$('#generateBtn').disabled=false}}
    async function renderHistory(){const h=$('#history');if(!state.data)return;const r=await api('/api/history');h.innerHTML='<div class="panel"><h2>History</h2><p class="muted">Find recent PicGen outputs and saved paths.</p></div><div class="gallery">'+r.runs.map(runCard).join('')+'</div>'}
    function runCard(r){const img=(r.images&&r.images[0])?'<img src="'+r.images[0].url+'&token='+token+'">':'';const plan=r.plan||{};return '<div class="run">'+img+'<div class="title">'+esc((r.prompt||'Untitled').slice(0,60))+'</div><div class="muted small">'+esc(new Date(r.created_at).toLocaleString())+' · '+esc(plan.provider||'')+' · '+esc(plan.preset||'')+'</div><div class="path small">'+esc(r.output_dir)+'</div><div class="actions" style="margin-top:8px"><button onclick="navigator.clipboard.writeText(\\''+esc(String(r.output_dir)).replaceAll("'","\\\\'")+'\\')">Copy folder</button></div></div>'}
